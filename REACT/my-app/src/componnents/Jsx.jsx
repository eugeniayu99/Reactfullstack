const Jsx = () => {
    ...
    // JSX에서 자주 사용하는 패턴
    // 1. 조건부 랜더링(삼항연산자)
    // JSX안에서 if문을 직접 쓸 수는 없고, 보통 삼항 연산자를 사용한다.
    const isLogin = true;

    // 2. 조건부 랜더링(AND연산자 &&)
    // 조건이 true면 랜더링을 한다.
    const hasMessage = true; // false면 랜더링이 되지 않는다.
    ...
    return(
        ...
        // 불필요한 div를 쓰고 싶지 않다면 Fragment(<></>)를 사용한다.
        <>
            {element}
            <h2>이름 : {name}</h2>
            <h2>나이 : {age}</h2>
            ...
            {/* CSS는 문자열이 아니라 JAvaScript 객체 형태호 써야한다. 속성들은 fontSize, backgroundColor처럼 카멜표현식으로 써야한다. */}
